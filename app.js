...
// JSONBlob Config (sem CORS issues)
var BLOB_URL = localStorage.getItem("blobUrl") || "";
var syncInProgress = false;

function cloudSave() {
  if (syncInProgress) return;
  if (!BLOB_URL) { createBlob(); return; }
  syncInProgress = true;
  var allData = {
    receitas: getData("receitas"),
    gastos: getData("gastos"),
    metas: getData("metas"),
    investimentos: getData("investimentos"),
    bancos: getData("bancos"),
    parcelas: getData("parcelas"),
    beneficios_rec: getData("beneficios_rec"),
    beneficios_gas: getData("beneficios_gas"),
    financiamento: JSON.parse(localStorage.getItem("financiamento") || "{}"),
    previdencia: JSON.parse(localStorage.getItem("previdencia") || "{}"),
    fgts: JSON.parse(localStorage.getItem("fgts") || "{}"),
    profile: getProfile()
  };
  fetch(BLOB_URL, {
    method: "PUT",
  ...
  }).then(function(r) { syncInProgress = false; console.log("Cloud save OK"); })
    .catch(function(e) { syncInProgress = false; console.error("Cloud save error", e); });
}

function createBlob() {
  var allData = { receitas: getData("receitas"), gastos: getData("gastos"), metas: getData("metas"), investimentos: getData("investimentos"), bancos: getData("bancos"), parcelas: getData("parcelas"),
    beneficios_rec: getData("beneficios_rec"),
    beneficios_gas: getData("beneficios_gas"),
    financiamento: JSON.parse(localStorage.getItem("financiamento") || "{}"),
    previdencia: JSON.parse(localStorage.getItem("previdencia") || "{}"),
    fgts: JSON.parse(localStorage.getItem("fgts") || "{}"), profile: getProfile() };
  fetch("https://jsonblob.com/api/jsonBlob", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(allData)
  }).then(function(r) {
    BLOB_URL = r.headers.get("Location") || r.headers.get("location");
    if (!BLOB_URL) BLOB_URL = "https://jsonblob.com/api/jsonBlob/" + r.headers.get("X-jsonblob");
    localStorage.setItem("blobUrl", BLOB_URL);
    console.log("Blob criado:", BLOB_URL);
  }).catch(function(e) { console.error("Blob create error", e); });
}

function cloudLoad() {
  if (!BLOB_URL) { console.log("Sem blob URL - usando localStorage"); renderDashboard(); return; }
  fetch(BLOB_URL)
    .then(function(r) { return r.json(); })
    .then(function(d) {
      if (d.receitas) localStorage.setItem("receitas", JSON.stringify(d.receitas));
      if (d.gastos) localStorage.setItem("gastos", JSON.stringify(d.gastos));
      if (d.metas) localStorage.setItem("metas", JSON.stringify(d.metas));
      if (d.investimentos) localStorage.setItem("investimentos", JSON.stringify(d.investimentos));
      if (d.bancos) localStorage.setItem("bancos", JSON.stringify(d.bancos));
      if (d.parcelas) localStorage.setItem("parcelas", JSON.stringify(d.parcelas));
      if (d.beneficios_rec) localStorage.setItem("beneficios_rec", JSON.stringify(d.beneficios_rec));
      if (d.beneficios_gas) localStorage.setItem("beneficios_gas", JSON.stringify(d.beneficios_gas));
      if (d.financiamento) localStorage.setItem("financiamento", JSON.stringify(d.financiamento));
      if (d.previdencia) localStorage.setItem("previdencia", JSON.stringify(d.previdencia));
      if (d.fgts) localStorage.setItem("fgts", JSON.stringify(d.fgts));
      if (d.profile) localStorage.setItem("profile", JSON.stringify(d.profile));
      console.log("Cloud load OK");
      renderDashboard();
    }).catch(function(e) { console.error("Cloud load error", e); renderDashboard(); });
}
...
function renderPage(page) {
  switch(page) {
    ...
    case "financiamento": renderFinanciamento(); break;
    case "previdencia": renderPrevidencia(); break;
    case "fgts": renderFGTS(); break;
...
function renderDashboard() {
  ...
  el.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center"><h1 class="page-title">Dashboard</h1><button class="btn" onclick="toggleHideValues()" style="font-size:0.75rem">' + (valuesHidden ? 'Mostrar Valores' : 'Ocultar Valores') + '</button></div>' +
    ...
    '</div>' +
    '<div class="section"><div class="section-title">Saldo Beneficios</div><div class="cards-grid">' +
    (function() { var s = getBenefSaldos(); return Object.keys(s).map(function(k) { return '<div class="card"><div class="card-label">' + k + '</div><div class="card-value ' + (s[k]>=0?'positive':'negative') + '">' + fmt(s[k]) + '</div></div>'; }).join(''); })() +
    '</div></div>' +
    '<div class="section"><div class="section-title">Patrimonio</div><div class="cards-grid">' +
    '<div class="card"><div class="card-label">Financiamento Carro</div><div class="card-value">' + (function(){var f=JSON.parse(localStorage.getItem('financiamento')||'{}');return (f.parcelas_pagas||0)+'/'+(f.parcelas_total||0)+' parcelas';})() + '</div></div>' +
    '<div class="card"><div class="card-label">Previdencia</div><div class="card-value positive">' + fmt(JSON.parse(localStorage.getItem('previdencia')||'{}').saldo||0) + '</div></div>' +
    '<div class="card"><div class="card-label">FGTS</div><div class="card-value positive">' + fmt(JSON.parse(localStorage.getItem('fgts')||'{}').saldo||0) + '</div></div>' +
    '</div></div>' +
...
function renderBeneficios() {
  var el = document.getElementById("page-beneficios");
  var beneficios = getData("beneficios_rec");
  var benefGastos = getData("beneficios_gas");
  var tipos = ["Vale Refeicao", "Vale Alimentacao", "Pontos Livelo"];

  var saldos = {};
  tipos.forEach(function(t) { saldos[t] = 0; });
  beneficios.forEach(function(b) { saldos[b.tipo] = (saldos[b.tipo] || 0) + Number(b.valor); });
  benefGastos.forEach(function(b) { saldos[b.tipo] = (saldos[b.tipo] || 0) - Number(b.valor); });

  el.innerHTML = '<h1 class="page-title">Beneficios</h1>' +
    '<div class="cards-grid">' +
    tipos.map(function(t) {
      var cls = saldos[t] >= 0 ? "positive" : "negative";
      return '<div class="card"><div class="card-label">' + t + '</div><div class="card-value ' + cls + '">' + fmt(saldos[t]) + '</div></div>';
    }).join("") +
    '</div>' +
    '<div class="section"><div class="section-title">Receber Beneficio</div>' +
    '<div class="form-grid">' +
    '<div class="form-group"><label>Tipo</label><select id="benef-tipo">' + tipos.map(function(t) { return "<option>" + t + "</option>"; }).join("") + '</select></div>' +
    '<div class="form-group"><label>Valor</label><input type="number" id="benef-valor" step="0.01"></div>' +
    '<div class="form-group"><label>Data</label><input type="date" id="benef-data" value="' + new Date().toISOString().split("T")[0] + '"></div>' +
    '</div><button class="btn" onclick="addBeneficio()">Receber</button></div>' +
    '<div class="section"><div class="section-title">Gastar Beneficio</div>' +
    '<div class="form-grid">' +
    '<div class="form-group"><label>Tipo</label><select id="benef-gas-tipo">' + tipos.map(function(t) { return "<option>" + t + "</option>"; }).join("") + '</select></div>' +
    '<div class="form-group"><label>Valor</label><input type="number" id="benef-gas-valor" step="0.01"></div>' +
    '<div class="form-group"><label>Descricao</label><input type="text" id="benef-gas-desc"></div>' +
    '<div class="form-group"><label>Data</label><input type="date" id="benef-gas-data" value="' + new Date().toISOString().split("T")[0] + '"></div>' +
    '</div><button class="btn" onclick="addBenefGasto()">Gastar</button></div>' +
    '<div class="section"><div class="section-title">Historico</div><div class="table-container"><table><thead><tr><th>Data</th><th>Tipo</th><th>Descricao</th><th>Valor</th><th></th></tr></thead><tbody>' +
    benefGastos.sort(function(a,b) { return b.data.localeCompare(a.data); }).map(function(g, i) {
      return '<tr><td>' + new Date(g.data).toLocaleDateString("pt-BR") + '</td><td>' + g.tipo + '</td><td>' + (g.desc || "") + '</td><td class="negative">' + fmt(g.valor) + '</td><td><button class="btn btn-danger" style="font-size:0.65rem;padding:4px 8px" onclick="delBenefGasto(' + i + ')">X</button></td></tr>';
    }).join("") +
    '</tbody></table></div></div>';
  applyHide();
}
...
function addBeneficio() {
  var b = getData("beneficios_rec");
  b.push({ tipo: document.getElementById("benef-tipo").value, valor: Number(document.getElementById("benef-valor").value), data: document.getElementById("benef-data").value });
  setData("beneficios_rec", b); cloudSave();
  renderBeneficios();
}

function addBenefGasto() {
  var g = getData("beneficios_gas");
  g.push({ tipo: document.getElementById("benef-gas-tipo").value, valor: Number(document.getElementById("benef-gas-valor").value), desc: document.getElementById("benef-gas-desc").value, data: document.getElementById("benef-gas-data").value });
  setData("beneficios_gas", g); cloudSave();
  renderBeneficios();
}

function delBenefGasto(i) { var g = getData("beneficios_gas").sort(function(a,b) { return b.data.localeCompare(a.data); }); g.splice(i,1); setData("beneficios_gas", g); cloudSave(); renderBeneficios(); }

function getBenefSaldos() {
  var beneficios = getData("beneficios_rec");
  var benefGastos = getData("beneficios_gas");
  var saldos = {"Vale Refeicao": 0, "Vale Alimentacao": 0, "Pontos Livelo": 0};
  beneficios.forEach(function(b) { saldos[b.tipo] = (saldos[b.tipo] || 0) + Number(b.valor); });
  benefGastos.forEach(function(b) { saldos[b.tipo] = (saldos[b.tipo] || 0) - Number(b.valor); });
  return saldos;
}
...
function addInvestimento() {
  ...
  setData("investimentos", inv); cloudSave();
  renderInvestimentos();
}
...
function renderFinanciamento() {
  var el = document.getElementById("page-financiamento");
  var fin = JSON.parse(localStorage.getItem("financiamento") || "{}");
  var totalParcelas = Number(fin.parcelas_total || 0);
  var parcPagas = Number(fin.parcelas_pagas || 0);
  var parcRestantes = totalParcelas - parcPagas;
  var valorParcela = Number(fin.valor_parcela || 0);
  var pct = totalParcelas > 0 ? Math.round((parcPagas / totalParcelas) * 100) : 0;

  el.innerHTML = '<h1 class="page-title">Financiamento do Carro</h1>' +
    '<div class="cards-grid">' +
    '<div class="card"><div class="card-label">Parcelas Pagas</div><div class="card-value positive">' + parcPagas + '/' + totalParcelas + '</div></div>' +
    '<div class="card"><div class="card-label">Parcelas Restantes</div><div class="card-value negative">' + parcRestantes + '</div></div>' +
    '<div class="card"><div class="card-label">Valor da Parcela</div><div class="card-value">' + fmt(valorParcela) + '</div></div>' +
    '<div class="card"><div class="card-label">Valor Restante</div><div class="card-value negative">' + fmt(valorParcela * parcRestantes) + '</div></div>' +
    '<div class="card"><div class="card-label">Total Pago</div><div class="card-value positive">' + fmt(valorParcela * parcPagas) + '</div></div>' +
    '<div class="card"><div class="card-label">Progresso</div><div class="card-value">' + pct + '%</div></div>' +
    '</div>' +
    '<div class="progress-bar" style="margin-bottom:2rem"><div class="progress-fill" style="width:' + pct + '%"></div></div>' +
    '<div class="section"><div class="section-title">Configurar Financiamento</div>' +
    '<div class="form-grid">' +
    '<div class="form-group"><label>Modelo do Carro</label><input type="text" id="fin-modelo" value="' + (fin.modelo || "") + '"></div>' +
    '<div class="form-group"><label>Valor da Parcela</label><input type="number" id="fin-parcela" step="0.01" value="' + (fin.valor_parcela || "") + '"></div>' +
    '<div class="form-group"><label>Total de Parcelas</label><input type="number" id="fin-total" value="' + (fin.parcelas_total || "") + '"></div>' +
    '<div class="form-group"><label>Parcelas Pagas</label><input type="number" id="fin-pagas" value="' + (fin.parcelas_pagas || "") + '"></div>' +
    '<div class="form-group"><label>Banco</label><input type="text" id="fin-banco" value="' + (fin.banco || "") + '"></div>' +
    '</div><button class="btn" onclick="saveFinanciamento()">Salvar</button> <button class="btn" style="background:var(--green)" onclick="pagarParcelaFin()">+1 Parcela Paga</button></div>';
  applyHide();
}

function saveFinanciamento() {
  var fin = { modelo: document.getElementById("fin-modelo").value, valor_parcela: Number(document.getElementById("fin-parcela").value), parcelas_total: Number(document.getElementById("fin-total").value), parcelas_pagas: Number(document.getElementById("fin-pagas").value), banco: document.getElementById("fin-banco").value };
  localStorage.setItem("financiamento", JSON.stringify(fin)); cloudSave();
  renderFinanciamento();
}

function pagarParcelaFin() {
  var fin = JSON.parse(localStorage.getItem("financiamento") || "{}");
  fin.parcelas_pagas = (Number(fin.parcelas_pagas) || 0) + 1;
  localStorage.setItem("financiamento", JSON.stringify(fin)); cloudSave();
  renderFinanciamento();
}

function renderPrevidencia() {
  var el = document.getElementById("page-previdencia");
  var prev = JSON.parse(localStorage.getItem("previdencia") || "{}");

  el.innerHTML = '<h1 class="page-title">Previdencia Privada</h1>' +
    '<div class="cards-grid">' +
    '<div class="card"><div class="card-label">Saldo Total</div><div class="card-value positive">' + fmt(prev.saldo || 0) + '</div></div>' +
    '<div class="card"><div class="card-label">Contribuicao Mensal</div><div class="card-value">' + fmt(prev.contribuicao || 0) + '</div></div>' +
    '<div class="card"><div class="card-label">Plano</div><div class="card-value" style="font-size:0.9rem">' + (prev.plano || "-") + '</div></div>' +
    '</div>' +
    '<div class="section"><div class="section-title">Atualizar Previdencia</div>' +
    '<div class="form-grid">' +
    '<div class="form-group"><label>Plano</label><input type="text" id="prev-plano" value="' + (prev.plano || "") + '"></div>' +
    '<div class="form-group"><label>Empresa</label><input type="text" id="prev-empresa" value="' + (prev.empresa || "") + '"></div>' +
    '<div class="form-group"><label>Saldo Atual</label><input type="number" id="prev-saldo" step="0.01" value="' + (prev.saldo || "") + '"></div>' +
    '<div class="form-group"><label>Contribuicao Mensal</label><input type="number" id="prev-contrib" step="0.01" value="' + (prev.contribuicao || "") + '"></div>' +
    '<div class="form-group"><label>Tipo (PGBL/VGBL)</label><input type="text" id="prev-tipo" value="' + (prev.tipo || "") + '"></div>' +
    '</div><button class="btn" onclick="savePrevidencia()">Salvar</button></div>';
  applyHide();
}

function savePrevidencia() {
  var prev = { plano: document.getElementById("prev-plano").value, empresa: document.getElementById("prev-empresa").value, saldo: Number(document.getElementById("prev-saldo").value), contribuicao: Number(document.getElementById("prev-contrib").value), tipo: document.getElementById("prev-tipo").value };
  localStorage.setItem("previdencia", JSON.stringify(prev)); cloudSave();
  renderPrevidencia();
}

function renderFGTS() {
  var el = document.getElementById("page-fgts");
  var fgts = JSON.parse(localStorage.getItem("fgts") || "{}");
  var multa40 = Number(fgts.saldo || 0) * 0.4;

  el.innerHTML = '<h1 class="page-title">FGTS</h1>' +
    '<div class="cards-grid">' +
    '<div class="card"><div class="card-label">Saldo FGTS</div><div class="card-value positive">' + fmt(fgts.saldo || 0) + '</div></div>' +
    '<div class="card"><div class="card-label">Multa 40%</div><div class="card-value">' + fmt(multa40) + '</div></div>' +
    '<div class="card"><div class="card-label">Empresa</div><div class="card-value" style="font-size:0.9rem">' + (fgts.empresa || "-") + '</div></div>' +
    '</div>' +
    '<div class="section"><div class="section-title">Atualizar FGTS</div>' +
    '<div class="form-grid">' +
    '<div class="form-group"><label>Saldo Atual</label><input type="number" id="fgts-saldo" step="0.01" value="' + (fgts.saldo || "") + '"></div>' +
    '<div class="form-group"><label>Empresa</label><input type="text" id="fgts-empresa" value="' + (fgts.empresa || "") + '"></div>' +
    '</div><button class="btn" onclick="saveFGTS()">Salvar</button></div>';
  applyHide();
}

function saveFGTS() {
  var fgts = { saldo: Number(document.getElementById("fgts-saldo").value), empresa: document.getElementById("fgts-empresa").value };
  localStorage.setItem("fgts", JSON.stringify(fgts)); cloudSave();
  renderFGTS();
}
